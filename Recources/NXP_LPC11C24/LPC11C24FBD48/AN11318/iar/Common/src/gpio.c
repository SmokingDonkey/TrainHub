...
extern volatile i2c_struct i2c;
...
void PIOINT0_IRQHandler(void)
{
  ...
  LPC_GPIO0->IC |= (1<<7);			// clear interrup status bit
  LPC_GPIO0->IE &= ~(0x1<<7);			// Disable the EXT int
  if (i2c.status & I2C_MSG)			//if this is a read only, then set STOP flag
  {
    i2c.status |= I2C_STOP;
  }
  return;		
