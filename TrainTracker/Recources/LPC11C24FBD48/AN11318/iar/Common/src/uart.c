...
// Surpress IAR Warning[Pa082]: undefined behavior: the order of volatile
...
void UARTInit(uint32_t baudrate)
{
  ...
  /* Enable the UART Interrupt for receive or transmit */
...
void myprintf(uint8_t *BufferPtr)
{
   uint32_t c=0,i;
  for(i=0;BufferPtr[i]!='\0';i++)
  {
     c++;
  }
 UARTSend(BufferPtr,c);
}
