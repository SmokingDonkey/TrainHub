...
__root const ADDR_TYPE RamBistMarchTestTable[NUM_RAM_MARCH_TEST_BLOCKS][2] = {
...
#define CCITT_FLASH_CRC { (CcittSig_t) 0x00002CE0 }
#define CRC16_FLASH_CRC { (Crc16Sig_t) 0xFD61 }
#define CRC32_FLASH_CRC { (Crc32Sig_t) 0xB949D070 }
  
#if (SELECTED_CRC_TYPE == CCITT_ALGO)
__root  const CcittSig_t  ccittFlashSignature @ "flashSignatureSection" = CCITT_FLASH_CRC;
#endif
  
#if (SELECTED_CRC_TYPE == CRC16_ALGO)
__root  const Crc16Sig_t  crc16FlashSignature @ "flashSignatureSection" = CRC16_FLASH_CRC;
#endif

#if (SELECTED_CRC_TYPE == CRC32_ALGO)
__root  const Crc32Sig_t  crc32FlashSignature @ "flashSignatureSection" = CRC32_FLASH_CRC;
#endif
