...
WDMOD_ADDR    EQU 0x40004000  ; addresses for the wd 
WDFEED_ADDR   EQU 0x40004008

Reset_Handler
        LDR     R1, =WDMOD_ADDR   ; load the mod address
        LDR     R2, =WDFEED_ADDR  ; load the pattern address
        MOVS    R5, #0xAA         ; load the pattern
        MVNS    R6, R5            ; load the negated pattern
        MOVS    R4, #0            ; load the mod value zero
        STR     R4, [R1]          ; set mod to zero
